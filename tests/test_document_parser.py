...
class FakeProvider:
    ...
    def parse_pdf_batch(self, input_pdf_path, *, volume, pdf_pages):
        self.calls += 1
...
def test_document_parser_raw_and_pageir_with_resume(tmp_path):
    ...
    raw_results = parser.parse_raw_for_volume(plan, source, volume=2)
    assert [r.status for r in raw_results] == ["raw_done", "raw_done", "raw_done"]
    assert provider.calls == 2

    # Resume: no additional provider calls.
    parser.parse_raw_for_volume(plan, source, volume=2)
    assert provider.calls == 2

    pageir_results = parser.build_pageir_for_volume(
        plan, FakeAdapter(), volume=2, source_pdf_path=source
    )
    assert [r.status for r in pageir_results] == ["done", "done", "done"]
    assert (tmp_path / "pageir" / "volume-2" / "page_000001.json").exists()

    # Resume PageIR generation.
    parser.build_pageir_for_volume(
        plan, FakeAdapter(), volume=2, source_pdf_path=source
    )
    assert state_store.is_done(
        ...
        model="fake-model",
        source_sha256=state_store.get(2, 3).source_sha256,
